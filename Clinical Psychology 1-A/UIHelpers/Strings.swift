...
struct AppStrings {
    ...
    static let itTrainingDescription = "Kelimeyi doğru bir şekilde tamamladıktan sonra karşınıza çıkacak soruyu cevaplarken lütfen sadece senaryonun içeriğinde yer alan bilgiyi göz önünde bulundurunuz. Evet cevabını vermek için ekrandaki sol ok (←) butonuna, Hayır cevabını vermek için ekrandaki sağ ok (→) butonuna basınız. Cevap vermeden bir sonraki aşamaya geçemeyeceksiniz. Eğer cevabınızdan emin değilseniz, lütfen size en uygun gelen tahmini yapınız. Şimdi bir alıştırma yapalım. Hazırsanız alıştırmaya başlamak için aşağıdaki BAŞLAT yazılı butona basın."
    static let itTrainingButtonText = "BAŞLAT"
    static let itTrainingFinishDescription = "Alıştırma bitti. Sonlandırmak ve çalışmaya başlamak içini aşağıdaki BİTİR VE DİĞER ADIMA GEÇ yazılı butona basın."
    static let itTrainingFinishButtonText = "BİTİR VE DİĞER ADIMA GEÇ"
    static let itStartDescription = "Kelimeyi doğru bir şekilde tamamladıktan sonra karşınıza çıkacak soruyu cevaplarken lütfen sadece senaryonun içeriğinde yer alan bilgiyi göz önünde bulundurunuz. Evet cevabını vermek için ekrandaki sol ok (←) butonuna, Hayır cevabını vermek için ekrandaki sağ ok (→) butonuna basınız. Cevap vermeden bir sonraki aşamaya geçemeyeceksiniz. Eğer cevabınızdan emin değilseniz, lütfen size en uygun gelen tahmini yapınız. Hazırsanız çalışmaya başlamak için aşağıdaki BAŞLAT yazılı butona basın."
